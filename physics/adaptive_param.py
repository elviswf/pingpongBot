...
class Adaptive_Para_Pingpong(nn.Module):
    ...
    def forward(self, input):
            ...
            vx, vy, vz = Variable(torch.FloatTensor(v_x_pre[15:16])),Variable(torch.FloatTensor(v_y_pre[15:16])), Variable(torch.FloatTensor(v_z_pre[15:16]))
            ...
            a_ba_x, a_ba_y, a_ba_z = Variable(torch.FloatTensor(a_ba_x_pre[15:16])), Variable(torch.FloatTensor(a_ba_y_pre[15:16])), Variable(
            ...
            vx, vy, vz = Variable(torch.FloatTensor([vx])), Variable(torch.FloatTensor([vy])), Variable(torch.FloatTensor([vz]))
            v_value = torch.sqrt(vx ** 2 + vy ** 2 + vz ** 2)
            ax, ay, az = Variable(torch.FloatTensor([0.])), -self.kd * v_value * vy, -self.g
            a_ba_x, a_ba_y, a_ba_z = Variable(torch.FloatTensor([0.])), Variable(torch.FloatTensor([0.])), Variable(torch.FloatTensor([0.]))
            a_ba_value = 0.
            #print('a')
            #print(ax,ay,az)
            #print('v')
            ...
            wy = (ay * (ax + self.kd * v_value * vx) - vy * (a_ba_x + self.kd * v_ba_value * vx + self.kd * v_value * ax)) / (self.km * (vz * ay - vy * az))
            ...
            if torch.min(torch.abs(self.wx), torch.abs(self.wy), torch.abs(self.wz)).data[0] > 5000 or\
                ...
                self.wx, self.wy, self.wz = Variable(torch.FloatTensor([0.])), Variable(torch.FloatTensor([0.])), Variable(torch.FloatTensor([0.]))
                self.wok = False
            #print p_x_pre[15]
            ...
            print()nt, bounce_loss, hit_loss, time_loss)
